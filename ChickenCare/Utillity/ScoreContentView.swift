...
struct ScoreContentView: View {
	@EnvironmentObject private var toolkitState: MeasurementToolkitState
...
private struct SessionListView: View {
	...
	var body: some View {
        if #available(iOS 16.0, *) {
            NavigationStack {
                Group {
                    if toolkitState.sessions.isEmpty {
                        EmptyStateView {
                            showingNewSession = true
                        }
                    } else {
                        List {
                            ForEach(toolkitState.sessions) { session in
                                NavigationLink {
                            ...
                            .onDelete { toolkitState.deleteSessions(at: $0) }
                        }
                ...
                .sheet(isPresented: $showingNewSession) {
                    NewSessionView(presets: toolkitState.presets) { session in
                        toolkitState.addSession(session)
                    }
                }
            }
        } else {
			...
			VStack(alignment: .leading, spacing: 20) {
				...
				VStack(alignment: .leading, spacing: 16) {
					...
					if session.canAddPlayer {
						Button {
							session.addPlayer()
							toolkitState.registerPlayerAdded()
						} label: {
			...
			HStack(spacing: 8) {
				ForEach(session.preset.scoringOptions) { option in
					Button {
						...
						toolkitState.registerScoreChange(delta: option.delta)
					} label: {
	...
	var body: some View {
        if #available(iOS 16.0, *) {
            NavigationStack {
                Form {
                    Section("Feedback") {
                        Toggle("Sound", isOn: $toolkitState.settings.soundEnabled)
                        Toggle("Vibration", isOn: $toolkitState.settings.vibrationEnabled)
                    }
...
private struct StatisticsView: View {
	...
	private var derived: DerivedStatistics { toolkitState.derivedMetrics }
	private var lifetime: AppStatistics { toolkitState.statistics }
...
final class MeasurementToolkitState: ObservableObject {
	static let shared = MeasurementToolkitState()
...
final class PersistenceController {
	...
	init(filename: String = "measurement_toolkit_state.json") {
...
#Preview {
	ScoreContentView()
		.environmentObject(MeasurementToolkitState())
}
